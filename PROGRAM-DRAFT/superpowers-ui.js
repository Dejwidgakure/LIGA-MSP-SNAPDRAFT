...
const SuperpowerUI = (()=>{
    ...
    function isDraftMutationLocked(){
        return Boolean(
            ...
            window.ThorUI?.isBusy?.() ||
            window.CollectorUI?.isBusy?.() ||
    ...
    function getDraftMutationLockReason(){
        ...
        if(window.ThorUI?.isBusy?.()) return window.ThorUI.getLockReason?.()||"Dokończ Test Godności Mjolnira.";
        if(window.CollectorUI?.isBusy?.()) return window.CollectorUI.getLockReason?.()||"Dokończ aktywną fazę Collection Collectora.";
